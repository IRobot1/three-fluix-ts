...
import { PointerEventType } from "./pointer-interaction";
...
export class KeyboardInteraction extends EventDispatcher {
  ...
  constructor(renderer: WebGLRenderer, options: KeyboardInteractionOptions = {}) {
    ...
    const materials = options.materials != undefined ? options.materials : new UIMaterials()
  ...
  add(...objects: InputField[]): this {
    if (arguments.length > 1) {
      ...
      object.addEventListener(PointerEventType.POINTERDOWN, () => {
        if (object.disabled) return
        object.active = true
      })

      // TODO: this event now on ThreeInteactive
      object.addEventListener(PointerEventType.POINTERMISSED, () => {
        object.active = false
      })

...
export class UIMaterials {
  ...
  private themesMap = new Map<string, UITheme>()
  ...
  constructor(theme?: UITheme) {
    if (theme)
      ...
      const defaultTheme: UITheme = {
        ...
        'geometry-edge': white,
        'line-edge': UIMaterialUtils.LineMaterial({ color: 0xffffff }),
      }
  ...
  addTheme(name: string, theme: UITheme) {
  ...
  getMaterial(type: string, purpose: string, parameters?: MaterialParameters): Material {
    ...
    if (parameters) {
      ...
      if (!purpose || !this.materialMap.has(key)) {
        let material
        if (type == 'line')
          material = UIMaterialUtils.LineMaterial(parameters);
        else
